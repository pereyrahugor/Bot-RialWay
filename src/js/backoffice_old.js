...
socket.on('connect', () => {
    console.log('Ô£à Conectado al servidor de tiempo real');
});

socket.on('new_message', (payload) => {
    console.log('­ƒôí Nuevo mensaje recibido:', payload);
...
socket.on('bot_toggled', (payload) => {
    console.log('­ƒôí Bot toggled:', payload);
    if (activeChatId === payload.chatId) {
    ...
    try {
        ...
        if (!activeChatId) {
            ...
            if (pendingChatId) {
                ...
                // Esperar un breve instante para asegurar que el DOM est├í listo
...
function handleSearch() {
    ...
    searchTimeout = setTimeout(() => {
        console.log('[Search] Disparando b├║squeda en servidor...');
        fetchChats(true);
    }, 500);
}
...
function renderChatList(listToRender = chats) {
    ...
    list.innerHTML = listToRender.map(chat => {
        ...
        const statusBadge = chat.bot_enabled 
            ? `<div style="text-align:right;"><span style="color: var(--accent); font-size: 0.75rem;">­ƒñû Bot</span><br/><span style="font-size:0.65rem; opacity:0.7;">${timeStr}</span></div>`
            : `<div style="text-align:right;"><span style="color: #f87171; font-size: 0.75rem;">­ƒæñ Humano</span><br/><span style="font-size:0.65rem; opacity:0.7;">${timeStr}</span></div>`;
...
async function checkPlatformVisibility() {
    try {
        ...
        // Mostrar/ocultar los tabs seg├║n configuraci├│n
        ...
        // Si hay m├ís de una plataforma activa, mostrar el tab "Todos"
        ...
        // Si la plataforma actual no est├í habilitada, cambiar a la primera disponible o 'all' si corresponde
...
function updateBotStatusText(enabled) {
    ...
    txt.innerText = isEnabled ? 'Bot Activo' : 'Intervenci├│n Humana';
...
function updateInputState(botEnabled) {
    ...
    // Normalizaci├│n estricta a booleano
    ...
    // Bloqueamos el input si el bot est├í activo para evitar interferencias
    ...
    if (isBotEnabled) {
        ...
        input.placeholder = "­ƒñû Bot activo - Desactiva para intervenir";
    } else {
        input.parentElement.style.borderColor = '#f87171';
        input.style.opacity = '1';
        input.placeholder = "Escribe un mensaje aqu├¡";
    }
}
...
async function fetchMessages(chatId, reset = false) {
    ...
    try {
        ...
        // Concatenar al inicio (los nuevos/viejos mensajes seg├║n el offset)
...
function generateMessageHtml(m) {
    ...
    if (isImageUrl && contentHtml) {
        ...
        contentHtml = `<div class="msg-file"><a href="${contentHtml}" target="_blank">­ƒôä Documento adjunto (${fileName})</a></div>`;
    }
...
async function sendMessage() {
    ...
    if (!content && !selectedFile) {
        console.log('ÔÜá´©Å Intento de env├¡o vac├¡o ignorado');
        isSending = false;
        return;
    }
    if (!activeChatId) {
        console.warn('ÔÜá´©Å No hay chat activo seleccionado');
        isSending = false;
        return;
    }

    console.log(`­ƒôñ Enviando mensaje a ${activeChatId}...`, { hasFile: !!selectedFile });
    const btn = document.getElementById('send-btn');
    ...
    try {
        ...
        if (!selectedFile) {
            // Enviar como JSON simple (m├ís confiable para texto)
        ...
        if (res.ok) {
            ...
            if (data.warning) {
                console.warn('ÔÜá´©Å Advertencia del servidor:', data.warning);
                alert('ÔÜá´©Å ' + data.warning);
            } else {
                console.log('Ô£à Mensaje enviado exitosamente');
            }
            input.value = '';
            input.placeholder = "Escribe un mensaje aqu├¡";
            selectedFile = null;
            ...
            console.error('ÔØî Error del servidor:', errorMsg);
            alert('Error al enviar mensaje: ' + errorMsg);
        }
    } catch (err) {
        console.error('ÔØî Error de red:', err);
...
async function loadCRMJump(chatId) {
    ...
    try {
        ...
        if (Array.isArray(tickets) && tickets.length > 0) {
            ...
            select.innerHTML = '<option value="">­ƒÜÇ Ver en CRM...</option>' + 
...
function jumpToCRM() {
    ...
    // Guardamos ambos IDs para que el CRM abra el modal espec├¡fico
...
async function saveCRMDetails() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            renderChatList();
            showToast('Ô£à Informaci├│n guardada correctamente');
        } else {
            showToast('ÔØî Error al guardar informaci├│n', 'error');
        }
    } catch (e) {
        console.error(e);
        showToast('ÔØî Error de conexi├│n', 'error');
    }
}
...
function showToast(message, type = 'success') {
    ...
    // Estilos inline r├ípidos para el toast si no est├ín en CSS
...
socket.on('ticket_updated', (payload) => {
    console.log('­ƒôí Ticket actualizado:', payload);
...
async function updateTicketStatus(ticketId, nuevoEstado) {
    try {
        ...
        if (result.success) {
            ...
            showToast('ÔØî Error al actualizar ticket: ' + (result.error || 'Desconocido'));
        }
    } catch (e) {
        console.error('Error updating ticket status:', e);
        showToast('ÔØî Error de conexi├│n al actualizar ticket');
    }
}
...
function goToTicketChat(chatId) {
    ...
    if (chat) {
        ...
        // Si el chat no est├í en la lista actual (paginaci├│n), forzamos b├║squeda
...
function openTicketModal() {
    if (!activeChatId) {
        showToast('ÔÜá´©Å Selecciona un chat primero', 'error');
        return;
    }
...
async function createTicket() {
    ...
    if (!titulo) {
        showToast('ÔÜá´©Å El t├¡tulo es obligatorio', 'error');
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            showToast('Ô£à Ticket generado correctamente');
            closeTicketModal();
            fetchPendingTicketsCount();
            ...
            showToast('ÔØî Error al generar ticket', 'error');
        }
    } catch (e) {
        console.error(e);
        showToast('ÔØî Error de conexi├│n', 'error');
    ...
    fetch('/api/backoffice/whatsapp/config?token=' + activeToken)
      ...
      .then(data => {
          if (!data.appId || !data.railwayProjectId) {
              showToast('ÔÜá´©Å Faltan credenciales de Meta o Project ID en el servidor', 'error');
              return;
          }
          
          // Abrir DuskCodes con par├ímetros de redirecci├│n din├ímica
      ...
      .catch(err => {
          console.error(err);
          showToast('ÔØî Error de conexi├│n al obtener configuraci├│n', 'error');
      });
}
...
async function checkMetaStatus() {
    try {
        ...
        // Mostrar bot├│n de masivos y actualizar panel solo si Meta est├í vinculado
                    ...
                    content.innerHTML = `
                        ...
                        <div>
                            ...
                            <p style="color: var(--text-muted); font-size: 1rem; margin-top: 15px; line-height: 1.6;">
                                Tu cuenta de <strong>WhatsApp Business</strong> est├í vinculada correctamente.
                            </p>
                        </div>
                        <div style="background: var(--bg-header); padding: 24px; border-radius: 20px; border: 1px solid var(--border); width: 100%; text-align: left;">
                            <h4 style="margin: 0 0 15px 0; color: #10b981; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 700;">Detalles de la conexi├│n:</h4>
                            <div style="font-size: 0.9rem; color: var(--text-main); line-height: 1.8;">
                                <div><strong>WABA ID:</strong> ${config.waba_id}</div>
                                <div><strong>ID de Tel├®fono:</strong> ${config.phone_number_id}</div>
                        ...
                        <button class="btn-primary" onclick="toggleBulkModal(); toggleMetaPanel();" style="width:100%; height:45px; display:flex; align-items:center; justify-content:center; gap:10px; background:#10b981; border:none; border-radius:12px; font-weight:600; cursor:pointer; color:white; margin-top: 20px;">
                            <i class="fas fa-layer-group"></i> Abrir Env├¡o Masivo
                        </button>
    ...
    try {
        ...
        if (availableTemplates.length > 0) {
            ...
            if (approved.length > 0) {
                options += '<optgroup label="Ô£à Aprobadas (listas para enviar)">';
                options += approved.map(t => `<option value="${t.name}">Ô£à ${t.name} (${t.language})</option>`).join('');
                options += '</optgroup>';
            }
            if (pending.length > 0) {
                options += '<optgroup label="ÔÅ│ Pendientes de revisi├│n">';
                options += pending.map(t => `<option value="${t.name}" disabled>ÔÅ│ ${t.name} (${t.language}) - En revisi├│n</option>`).join('');
                options += '</optgroup>';
            }
            if (rejected.length > 0) {
                options += '<optgroup label="ÔØî Rechazadas">';
                options += rejected.map(t => `<option value="${t.name}" disabled>ÔØî ${t.name} (${t.language}) - Rechazada</option>`).join('');
                options += '</optgroup>';
            }
            if (approved.length === 0) {
                options += '<option value="" disabled>ÔÜá´©Å No hay plantillas aprobadas para enviar</option>';
            }
        } else {
            options += '<option value="" disabled>Sin plantillas</option>';
        }
        
        // La opci├│n de crear nueva debe estar siempre disponible
        options += '<option value="NEW" style="color:#10b981; font-weight:700;">Ô×ò Crear nueva plantilla...</option>';
        ...
        showToast('ÔØî No se pudieron cargar las plantillas de Meta', 'error');
    }
}
...
async function submitTemplateForReview() {
    ...
    if (!name || !text) {
        showToast('ÔÜá´©Å Nombre y cuerpo son obligatorios', 'error');
        return;
    }

    // Validar nombre (min├║sculas y guiones bajos)
    if (!/^[a-z0-9_]+$/.test(name)) {
        showToast('ÔÜá´©Å El nombre solo permite min├║sculas y guiones bajos', 'error');
        return;
    }
    ...
    for (const varName of varNames) {
        ...
        if (!value) {
            showToast(`ÔÜá´©Å Complet├í el ejemplo para la variable {{${varName}}}`, 'error');
            return;
        }
        examples.push(value);
    }
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showToast('Ô£à Plantilla enviada a Meta para revisi├│n. Aparecer├í como ÔÅ│ PENDIENTE hasta que la aprueben.');
            cancelTemplateCreation();
            loadTemplates();
        } else {
            showToast('ÔØî Error: ' + (data.error || 'No se pudo crear'), 'error');
        }
    } catch (e) {
        console.error(e);
        showToast('ÔØî Error de conexi├│n al crear plantilla', 'error');
    } finally {
        btn.disabled = false;
        btn.innerHTML = 'Enviar a Revisi├│n';
    }
}
...
// Vincular detecci├│n de variables al textarea
...
async function startBulkSend() {
    ...
    if (!templateName || fileInput.files.length === 0) {
        showToast('ÔÜá´©Å Seleccione una plantilla y suba un archivo Excel', 'error');
        return;
    }
    ...
    try {
        ...
        if (res.status === 202) {
            // El proceso inici├│ en segundo plano
            statusText.innerText = 'Ô£à Proceso iniciado en segundo plano. Los mensajes se enviar├ín gradualmente.';
            progressBar.style.width = '100%';
            progressBar.style.background = '#10b981';
            showToast('­ƒÜÇ Env├¡o masivo iniciado correctamente');
            
            setTimeout(() => {
                toggleBulkModal();
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-paper-plane" style="margin-right:8px;"></i> Iniciar Env├¡o';
            ...
            const data = await res.json();
            throw new Error(data.error || 'Error al iniciar env├¡o');
        }
    } catch (e) {
        console.error('[Bulk] Error:', e);
        statusText.innerText = 'ÔØî Error: ' + e.message;
        progressBar.style.background = '#ef4444';
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-paper-plane" style="margin-right:8px;"></i> Reintentar';
        showToast('ÔØî Error al iniciar env├¡o masivo', 'error');
    }
}
