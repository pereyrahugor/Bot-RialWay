...
const SHEET_ID = process.env.SHEET_ID_PROD_VENTA ?? "";
const SHEET_RANGE = process.env.SHEET_PROD_VENTA_RANGE ?? "";
const VECTOR_STORE_ID = process.env.VECTOR_STORE ?? "";
const TXT_PATH = path.join("temp", "base2.json");
let currentFileId: string | null = null;

// Verificar que las variables de entorno estén definidas
if (!SHEET_ID || !SHEET_RANGE) {
    throw new Error("❌ Las variables de entorno SHEET_ID_PROD_VENTA y SHEET_PROD_VENTA_RANGE deben estar definidas.");
}
...
export async function getSheet2() {
    try {
...
async function deleteOldFiles() {
    try {
        console.log("🗑️ Eliminando archivo anterior del vector store relacionado con base2.json...");

        const files = await openai.files.list();
        for (const file of files.data) {
            if (file.filename === "base2.json") {
...
function deleteTemporaryFiles() {
    try {
        ...
        const files = glob.sync(path.join("temp", "base2.json"));
