...
if (!supabaseUrl || !supabaseKey) {
    console.warn('⚠️ [HistoryHandler] Sin credenciales Supabase — base de datos desactivada. Configura SUPABASE_URL y SUPABASE_KEY.');
}
        ...
        try {
            ...
            if (!currentSettings || currentSettings.length === 0) {
                ...
                if (masterSettings && masterSettings.length > 0) {
                    const settingsToInsert = masterSettings.map((s: any) => ({
                        project_id: currentProjectId,
                        key: s.key,
                        value: s.value,
                        updated_at: new Date().toISOString()
