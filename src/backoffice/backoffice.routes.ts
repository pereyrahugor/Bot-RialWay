...
/** Envía un mensaje de texto o archivo al chat en WhatsApp (soporta imagen, video, documento).
 *  Guarda el mensaje en el historial, inyecta en el thread de OpenAI e inhabilita el bot para ese chat.
 *  Usada por la ruta POST /api/backoffice/send-message dentro de registerBackofficeRoutes. */
...
/** Escribe una respuesta JSON directamente en el objeto res de Polka (sin Express res.json).
 *  Usado internamente en processBulkTemplate y otras rutas que no usan el compatibility layer. */
...
/** Procesa el envío masivo de plantillas WhatsApp (HSM) a partir de un Excel.
 *  Lee el archivo con XLSX, detecta parámetros nombrados o posicionales, descarga y sirve medios localmente,
 *  comprime videos con ffmpeg si superan 15 MB y envía a cada número vía provider.sendTemplate.
 *  Responde 202 inmediatamente y procesa el lote en segundo plano. */
...
/** Registra todas las rutas HTTP del backoffice en la instancia de Polka.
 *  Incluye auth, usuarios, chats, mensajes, tickets, leads, etiquetas, config, templates de Meta,
 *  importación/exportación de contactos y el proxy a uploads.
 *  Recibe las dependencias (adapterProvider, groupProvider, HistoryHandler, openaiMain, upload)
 *  para pasarlas a processSendMessage, processBulkTemplate y processImportExcel. */
export const registerBackofficeRoutes = (app: any, deps: BackofficeDependencies) => {
    ...
    app.post('/api/backoffice/update-prompt', systemConfigAuth, bodyParser.json(), async (req: any, res: any) => {
        ...
        try {
            ...
            const { getOpenAI } = await import("../ai/openaiHelper");
            ...
            if (assistantId && dynamicOpenai) {
                ...
                const { syncAssistantTools } = await import("../ai/openaiHelper");
 ...
 *  Lee columnas phone, name y tags; crea los chats inexistentes y asigna etiquetas por nombre.
 *  Usada por la ruta POST /api/backoffice/chats/import dentro de registerBackofficeRoutes. */
