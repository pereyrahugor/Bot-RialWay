/* global logout */
// Obtiene el estado del bot desde /api/dashboard-status y actualiza la UI de conexión.
// Distingue proveedor Meta activo, Meta en curso, Baileys y modo dual (adapter + group).
// Llama a renderProviderStatus para los providers tipo Baileys. Se autolanza cada 15 s.
...
// Renderiza el estado de un proveedor Baileys en los elementos #session-status, #qr-section y #session-info.
// Llamada por fetchStatus cuando el proveedor no es Meta o como respaldo.
...
// Lee el estado global del bot desde /api/backoffice/settings/bot-status y actualiza el toggle.
// Se autolanza cada 30 s. Conecta con el listener del botToggle.
...
// Envía el nuevo estado habilitado/deshabilitado a /api/backoffice/settings/toggle-bot al cambiar el toggle.
if (botToggle) {
...
// Solicita un reinicio suave del motor del bot vía /api/backoffice/system/restart (sin borrar la sesión).
// Útil para aplicar cambios de Meta o Google Sheets sin desconectar WhatsApp.
if (reloadBtn) {
...
// Muestra el modal de confirmación de reinicio completo (borra sesión + reinicia Railway).
if (goResetBtn) {
...
// Cierra el modal de reinicio completo sin ejecutar ninguna acción.
if (confirmNo) {
...
// Ejecuta el reinicio completo: llama a /api/delete-session y luego /api/restart-bot,
// luego recarga la página. Equivalente al flujo de webreset.js pero dentro del modal de conexión.
if (confirmSi) {
