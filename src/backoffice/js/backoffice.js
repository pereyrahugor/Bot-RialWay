...
// Lleva el scroll del contenedor #messages al fondo. Llamada por fetchMessages y socket new_message.
...
// Ordena el array global `chats` por last_message_at descendente. Llamada por fetchChats y socket new_message.
function sortChats() {
...
// Serializa la lista de chats en localStorage con timestamp. Llamada por fetchChats y socket new_message.
...
// Lee la lista de chats del localStorage si tiene menos de 10 minutos; de lo contrario retorna null.
// Llamada por fetchChats para mostrar datos instantáneos mientras llega la respuesta del servidor.
...
// Solicita la lista de chats a /api/backoffice/chats con paginación, búsqueda y filtro de tag/plataforma.
// Si refresh=true reinicia el offset; soporta infinite scroll añadiendo al array existente.
// Llama a renderChatList y, si hay un chat pendiente en localStorage, a selectChat.
...
// Carga todas las etiquetas del bot desde /api/backoffice/tags y las pasa a
// renderTagManager, renderFilterDropdown y renderBulkFilterDropdown.
...
// Rellena el select de filtro de etiquetas para el envío masivo (#bulk-filter-tags)
// conservando las opciones ya seleccionadas. Llamada por fetchBotTags.
...
// Debounce de 500 ms sobre el input de búsqueda; dispara fetchChats(true) para buscar en el servidor.
...
// Rellena el select de filtro rápido por etiqueta (#filter-tag) manteniendo el valor actual.
// Llamada por fetchBotTags.
...
// Formatea una fecha ISO en "HH:MM", "Ayer" o "DD/MM" según cuán reciente es.
// Llamada por renderChatList para la columna de tiempo en cada fila.
...
// Genera el HTML de todos los chats en #chat-list con avatares, etiquetas, estado de bot y unread count.
// Llama a observeAvatars para activar lazy-load de imágenes de perfil.
...
// Usa IntersectionObserver para cargar las fotos de perfil solo cuando son visibles en #chat-list.
// Guarda los URLs exitosos en avatarCache. Llamada por renderChatList.
...
// Cambia la plataforma activa (whatsapp / instagram / messenger / all) y recarga la lista de chats.
// Actualiza los tabs del DOM y llama a fetchChats(true).
...
// Consulta /api/backoffice/settings para saber qué plataformas están habilitadas
// y muestra/oculta los tabs correspondientes en la UI. Llama a switchPlatform si la actual fue desactivada.
...
// Activa un chat: actualiza el header, el toggle del bot, las etiquetas, el panel CRM
// y carga los mensajes llamando a fetchMessages. También dispara loadCRMJump.
...
// Muestra las etiquetas del chat activo como pills de color en #active-chat-tags.
// Llamada por selectChat y addTagToChat/removeTagFromChat.
...
// Actualiza el texto de estado (#bot-status-text) entre "Bot Activo" e "Intervención Humana".
// Llamada por selectChat, toggleBot y socket bot_toggled.
...
// Habilita o deshabilita el input de mensaje y el botón de envío según si el bot está activo.
// Llamada por selectChat, toggleBot, sendMessage y socket bot_toggled.
...
// Carga mensajes paginados de /api/backoffice/messages/:chatId.
// reset=true reinicia el scroll al fondo; false añade mensajes anteriores arriba (infinite scroll).
// Llama a renderMessages y mantiene la posición de scroll.
...
// Genera el HTML de todos los mensajes en #messages con separadores de fecha.
// Llama a generateMessageHtml para cada mensaje. Llamada por fetchMessages y socket new_message.
...
// Devuelve el HTML de una burbuja de mensaje según su tipo (text / image / video / document).
// Llamada por renderMessages para cada elemento de allMessages.
...
// Envía el nuevo estado del bot a /api/backoffice/toggle-bot para el chat activo.
// Luego actualiza updateBotStatusText, updateInputState y renderChatList.
...
// Guarda el archivo seleccionado en selectedFile y actualiza el placeholder del input de mensaje.
// Conecta con el input file #file-input; sendMessage usa selectedFile al enviarse.
...
// Envía un mensaje de texto o archivo al chat activo vía POST /api/backoffice/send-message.
// Usa JSON para texto y FormData para archivos. Llama a updateInputState y deshabilita el bot toggle.
...
// Muestra u oculta el panel lateral del CRM. Cuando se abre, llama a populateCRMFields y renderTagManager.
...
// Busca tickets asociados al chat en /api/backoffice/tickets?chatId= y muestra el select
// de salto al CRM (#crm-lead-jump). Llamada por selectChat.
...
// Lee el ticket seleccionado en #crm-lead-jump, guarda los IDs en localStorage y navega a /crm.
// El CRM lee esos valores en DOMContentLoaded para abrir el modal del ticket directamente.
function jumpToCRM() {
    const select = document.getElementById('crm-lead-jump');
...
// Rellena los inputs del panel CRM (nombre, email, notas, cuit, etc.) con datos del chat activo.
// Llamada por selectChat y toggleCRMPanel.
...
// Guarda los datos del panel CRM vía PUT /api/backoffice/chat/:id/contact y muestra un toast.
// Actualiza el nombre en el header del chat y llama a renderChatList.
...
// Muestra una notificación flotante (toast) en la parte inferior de la pantalla por 3 segundos.
// type puede ser 'success' (verde) o 'error' (rojo). Llamada por múltiples funciones.
...
// Crea una nueva etiqueta vía POST /api/backoffice/tags y refresca los dropdowns con fetchBotTags.
...
// Elimina una etiqueta vía DELETE /api/backoffice/tags/:id, refresca los dropdowns y
// quita la etiqueta de los chats en memoria sin recargar el servidor.
...
// Asigna la etiqueta tagId al chat activo vía POST /api/backoffice/chats/:id/tags.
// Actualiza el array chat.tags en memoria y llama a renderActiveChatTags y renderTagManager.
...
// Quita la etiqueta tagId del chat activo vía DELETE /api/backoffice/chats/:id/tags/:tagId.
// Actualiza el array en memoria y refresca renderActiveChatTags y renderTagManager.
...
// Renderiza el editor de etiquetas y la lista de etiquetas asignables al chat activo.
// Las etiquetas ya asignadas muestran un check; las disponibles llaman a addTagToChat/removeTagFromChat.
...
// Obtiene el número de tickets pendientes de tipo "Asistencia Externa" y actualiza el badge #tickets-badge.
...
// Cambia el filtro de tickets (pending / Cerrado), actualiza los tabs y llama a fetchTickets.
...
// Carga la lista de tickets de /api/backoffice/tickets y los renderiza en #tickets-list.
// Usa currentTicketsFilter para filtrar por estado. Llamada por setTicketsFilter y socket ticket_updated.
...
// Actualiza el estado de un ticket vía PUT /api/backoffice/tickets/:id.
// Llama a fetchPendingTicketsCount y fetchTickets para refrescar la UI.
...
// Cierra el panel de tickets y abre el chat asociado al ticket.
// Si el chat no está cargado (paginación), busca por número y luego llama a selectChat.
...
// Abre el modal de creación de ticket para el chat activo. Requiere un chat seleccionado.
...
// Envía el nuevo ticket vía POST /api/backoffice/tickets, cierra el modal y refresca el badge.
...
// Carga los leads (chats con datos editados) desde /api/backoffice/leads y los renderiza en #leads-list.
// Cada lead llama a selectLead al hacer click.
...
// Cierra el panel de leads y abre el chat del lead seleccionado llamando a selectChat.
function selectLead(chatId) {
    realToggleLeads();
    selectChat(chatId);
}

// Abre/cierra el panel de leads cerrando primero los demás paneles.
// Expuesto como window.realToggleLeads para que crm-common.js lo delegue desde cualquier página.
...
// Abre/cierra el panel de tickets cerrando los demás paneles. Al abrir, carga los tickets pendientes.
// Expuesto como window.realToggleTickets para crm-common.js.
...
// Abre/cierra el panel de Meta cerrando los demás paneles. Al abrir llama a checkMetaStatus.
// Expuesto como window.toggleMetaPanel y también lo usa crm-common.js como window.realToggleMeta.
...
// Inicia el flujo de alta de Meta abriendo una ventana emergente hacia duskcodes.com.ar/meta-auth
// con el railwayProjectId, metaAppId y redirectUri del servidor.
...
// Asigna el chat activo al usuario actual vía POST /api/backoffice/chat/assign.
// Extrae el userId del token sub:ID. Llama a fetchChats(true) para refrescar la lista.
...
// Cierra el ticket seleccionado en #crm-lead-jump vía PUT estado=Cerrado.
// Llama a fetchPendingTicketsCount y loadCRMJump para refrescar la UI.
...
// Reabre el ticket activo cambiando su estado a "Abierto" vía PUT.
// Llama a fetchPendingTicketsCount y loadCRMJump.
...
// Elimina permanentemente el ticket activo vía DELETE /api/backoffice/tickets/:id.
// Llama a fetchPendingTicketsCount y loadCRMJump.
...
// Invierte el estado del toggle del bot, delegando en toggleBot.
// Es el punto de entrada del botón de intervención manual del operador.
...
// Consulta /api/backoffice/whatsapp/config para saber si Meta está vinculado.
// Si está conectado muestra los detalles de WABA y el botón de envío masivo;
// si no, muestra el panel de onboarding con launchMetaOnboarding.
