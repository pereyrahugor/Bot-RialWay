...
// Carga el orden de columnas guardado en /api/backoffice/get-setting?key=CRM_COLUMNS.
// Si no existe, usa las columnas por defecto y llama a renderBoard.
...
// Persiste el array `columns` actual en el servidor con la clave CRM_COLUMNS.
// Llamada por initDragAndDrop (reordenar columnas) y saveColumnName.
...
// Carga en paralelo leads, tickets abiertos y metadata (CRM_METADATA) desde el servidor.
// Filtra tickets de tipo "Asistencia Externa" y luego llama a renderBoard. Expuesta globalmente.
...
// Genera el HTML de todas las columnas del tablero Kanban y las inserta en #kanban-board-inner.
// Llama a distributeCards y initDragAndDrop al terminar.
...
// Distribuye cada ticket en la columna que indica crmData[ticket.id].columnId.
// Crea los elementos con createCardElement y luego llama a updateCounters y checkAlertsVisual.
...
// Construye y devuelve el elemento DOM de una tarjeta Kanban con datos del ticket, lead y metadata.
// El click en la tarjeta navega al chat en /backoffice. Los botones llaman a confirmCloseTicket y openCardModal.
...
// Inicializa SortableJS en los contenedores de tarjetas (drag entre columnas) y en el board (reordenar columnas).
// Al soltar una tarjeta llama a saveCRMMetadata; al reordenar columnas llama a saveCRMState.
...
// Persiste el objeto `crmData` (posición en columna, alertas, notas) con la clave CRM_METADATA.
// Llamada por initDragAndDrop, el submit del formulario de edición y confirmCloseTicket.
...
// Actualiza el badge de cantidad de tarjetas en el encabezado de cada columna.
// Llamada por distributeCards y initDragAndDrop.
...
// Abre el modal de edición de tarjeta y rellena todos los campos con datos del ticket, lead y metadata.
// Expuesta globalmente. El formulario #card-edit-form llama a saveCRMMetadata y syncCRM al guardarse.
...
// Agrega un textarea de nota adicional al modal de edición de tarjeta.
// Las notas se concatenan a mainNotes al guardar el formulario.
window.addNewNoteUI = () => {
...
// Abre un chat de WhatsApp con el número del lead actual en wa.me.
// Lee el número desde #edit-lead-phone y limpia los caracteres no numéricos.
...
// Cierra el modal de edición de tarjeta.
...
// Muestra el modal de creación manual de lead. El form #new-lead-form llama a la API manual-lead.
function openNewLeadModal() { document.getElementById('new-lead-modal').classList.add('active'); }
// Cierra el modal de creación manual de lead.
...
// --- Gestión de Columnas ---
let editingColIdx = -1;
// Abre el modal de renombrado de columna para la columna con id colId (no aplica a columnas fijas).
...
// Cierra el modal de edición de columna y resetea editingColIdx.
function closeColumnModal() {
    document.getElementById('column-modal').classList.remove('active');
    editingColIdx = -1;
}

// Guarda el nuevo nombre de la columna en el array `columns`, persiste con saveCRMState y re-renderiza.
...
// Agrega una nueva columna al tablero (máx. 10) y abre inmediatamente el modal de renombrado.
...
// Elimina la columna en edición. Si tiene tarjetas, pide confirmación y las mueve a UNASSIGNED.
// Llama a saveCRMMetadata, saveCRMState y renderBoard.
...
// --- Utilidades ---
// Devuelve el color hex correspondiente a la prioridad (Alta/Media/Baja). Usada por createCardElement.
...
// Retorna la clase CSS de alerta ("alert-active" / "alert-today" / "") según la fecha vs hoy.
// Usada por createCardElement y checkAlertsVisual.
...
// Recorre todos los elementos de alerta en el DOM y actualiza su clase CSS según la fecha actual.
// Llamada en DOMContentLoaded y cada 60 segundos vía setInterval.
...
// Formatea una fecha ISO a "DD/MM" para mostrar en las tarjetas. Usada por createCardElement.
...
// Muestra una notificación flotante (toast) en la parte inferior por 3 s.
// type puede ser 'success' (verde) o 'error' (rojo). Idéntica a la de backoffice.js.
...
// --- Cierre de Leads ---
// Cierra un ticket: guarda la fecha de cierre en crmData, actualiza su estado vía PUT y refresca el tablero.
...
// Abre el historial de leads cerrados. Solo muestra tickets con estado "Cerrado" Y fecha de cierre en crmData.
...
// Cierra el modal del historial de leads cerrados.
...
// Carga la lista de usuarios del equipo desde /api/backoffice/users y llama a renderUsersList y renderAssigneeSelect.
// Solo se ejecuta si isAdmin es true.
...
// Renderiza la lista de usuarios del equipo en #users-list con su rol.
// Llamada por loadTeam y saveNewUser.
...
// Rellena el select #edit-lead-assignee con los usuarios del equipo conservando el valor actual.
// Llamada por loadTeam.
...
// Abre el modal de creación de nuevo usuario y recarga la lista del equipo.
window.openNewUserModal = () => {
    document.getElementById('modal-users').classList.add('active');
    loadTeam();
};

// Crea un nuevo usuario del equipo vía POST /api/backoffice/users y refresca la lista con loadTeam.
