...
export async function rechargeUserSelenium(
    ...
    if (!localDriver) {
        ...
        try {
            ...
            if (!logged) {
                console.error("❌ [Cas-EPC] Fallo en la autenticación del administrador para recarga.");
                await localDriver.quit();
                return false;
            }
        } catch (authErr: any) {
            console.error("❌ [Cas-EPC] Excepción durante el login para recarga:", authErr.message || authErr);
            if (localDriver) {
    ...
    try {
        ...
        console.error("❌ Error en el proceso de recarga de usuario de Selenium para Cas-EPC:", error.message || error);
