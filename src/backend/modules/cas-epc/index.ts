...
export const casEpcModule = {
  ...
  tools: {
    ...
    CREAR_JUGADOR: async (args: any, context: any) => {
      ...
      console.log(`[casEpcModule] 👤 Invocando CREAR_JUGADOR para: "${nombre}" | recharge: ${recharge} | monto: ${monto}`);
      
      const res = await createUserSelenium(nombre, recharge);
      if (res) {
          if (recharge && monto > 0 && res.driver) {
              console.log(`[casEpcModule] 🔗 Reutilizando driver para recarga automática de $${monto} a ${res.username}...`);
  ...
  openAiTools: [
    {
      ...
      "function": {
        "name": "CREAR_JUGADOR",
        "description": "Crea una nueva cuenta de jugador en la plataforma Cas - EPC, con la opción de realizar una carga inicial automática.",
        "parameters": {
          "type": "object",
          "properties": {
            "nombre": {
