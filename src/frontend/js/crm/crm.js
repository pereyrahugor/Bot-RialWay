    ...
    for (const t of rawTags) {
        ...
        if (typeof t === 'object' && t.name) {
            ...
            if (found) {
                ...
                tagObj = { id: t, name: t, color: '#6366f1' };
            }
        }
        if (tagObj && tagObj.name) {
            const key = String(tagObj.id || tagObj.name).trim().toLowerCase();
            if (!seen.has(key)) {
                seen.add(key);
                result.push(tagObj);
            }
...
function distributeCards() {
    ...
    filteredTickets.forEach(ticket => {
        const lead = findLeadForTicket(ticket);
...
function createCardElement(ticket, lead, metadata) {
    ...
    card.onclick = (e) => {
        if (e.target.closest('button') || e.target.closest('.card-tag-count') || e.target.closest('.card-tags')) return;
    ...
    const dbAlertDate = actualLead?.crm_due_date ? actualLead.crm_due_date.split('T')[0] : null;
    ...
    const tagsHtml = (isVisible('crm-tags') && tagCount > 0) ? `
        <div class="card-tags" onclick="event.stopPropagation(); openCardModal('${ticket.id}')">
            ${assignedTags.map(tag => {
                const textColor = (typeof window.getTagTextColor === 'function') 
                    ? window.getTagTextColor(tag.color) 
                    : (tag.color === '#000' || tag.color === '#000000' || tag.color === 'black' ? '#ffffff' : '#ffffff');
                return `
                    <span class="card-tag" style="background:${tag.color || '#6366f1'} !important; color:${textColor} !important; cursor:pointer;" title="${escapeCRMText(tag.name)}">
...
function _setupCRMFormHandlers() {
    ...
    if (cardForm) cardForm.onsubmit = async (e) => {
        ...
        const rawCuit = document.getElementById('edit-lead-cuit').value;
        const rawEmpresa = document.getElementById('edit-lead-company').value;
    ...
    window.checkAndFetchSharedNotes = async () => {
        ...
        const cleanCuit = typeof window.normalizeCuitDni === 'function' ? window.normalizeCuitDni(cuitInput?.value) : (cuitInput?.value || '').trim();
        const cleanEmpresa = typeof window.normalizeEmpresa === 'function' ? window.normalizeEmpresa(empresaInput?.value) : (empresaInput?.value || '').trim();
