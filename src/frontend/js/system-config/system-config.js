...
async function _initSystemConfigPage() {
    ...
    function updateClientSlugVisibility() {
        ...
        if (trustContainer) {
            ...
            if (val === 'trust' && typeof window.loadTrustBaseStatus === 'function') {
                window.loadTrustBaseStatus();
            }
        }
    }
    ...
    variablesForm.addEventListener('submit', async (e) => {
        ...
        const credentialFields = ['ADMIN_USER', 'ADMIN_PASS', 'SUPERVISOR_API_KEY', 'GANAMOSNET_WEBSHARE_API_KEY', 'GANAMOSNET_PROXY_URL'];
    ...
    // ==========================================
    // EXCLUSIVO SLUG: TRUST (TANGO GESTIÓN) - EXCEL PEDIDOS
    // ==========================================
    let selectedTrustExcelFile = null;

    window.handleTrustFileSelected = (input) => {
        const file = input?.files?.[0];
        const filenameSpan = document.getElementById('trust-selected-filename');
        const uploadBtn = document.getElementById('trust-upload-btn');
        const feedback = document.getElementById('trust-upload-feedback');
        if (feedback) feedback.style.display = 'none';

        if (file) {
            selectedTrustExcelFile = file;
            if (filenameSpan) {
    ...
    window.loadTrustBaseStatus = async () => {
        const token = getSafeToken();
        const badge = document.getElementById('trust-base-status-badge');
        const metaInfo = document.getElementById('trust-base-meta-info');
        const headersPreview = document.getElementById('trust-base-headers-preview');
        const headersText = document.getElementById('trust-base-headers-text');

        try {
            const res = await fetch(`/api/backoffice/trust/base-pedido-status?token=${token}`);
            const data = await res.json();

            if (data.success && data.hasData) {
                if (badge) {
                    badge.textContent = `✅ ${data.totalRows} artículos cargados`;
                    badge.style.background = 'rgba(16, 185, 129, 0.2)';
                    badge.style.color = '#10b981';
                }
                if (metaInfo && data.lastUpdated) {
                    const date = new Date(data.lastUpdated).toLocaleString('es-AR');
                    metaInfo.textContent = `Actualizado: ${date}`;
                }
                if (headersPreview && headersText && Array.isArray(data.headers) && data.headers.length > 0) {
                    headersPreview.style.display = 'block';
                    headersText.textContent = data.headers.join(', ');
                }
            } else {
                if (badge) {
                    badge.textContent = '⚠️ Sin base cargada';
                    badge.style.background = 'rgba(239, 68, 68, 0.2)';
                    badge.style.color = '#ef4444';
                }
                if (metaInfo) metaInfo.textContent = 'Aún no se ha subido ningún Excel.';
                if (headersPreview) headersPreview.style.display = 'none';
            }
        } catch (err) {
            console.error('[Trust] Error consultando status de base:', err);
            if (badge) {
    ...
    window.uploadTrustExcelBase = async () => {
        if (!selectedTrustExcelFile) {
            window.swalAlert?.('Atención', 'Por favor selecciona un archivo Excel primero.', 'warning');
            return;
        }
        ...
        if (uploadBtn) {
            ...
            uploadBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Subiendo y procesando...';
        }

        try {
            const formData = new FormData();
            formData.append('file', selectedTrustExcelFile);

            const res = await fetch(`/api/backoffice/trust/upload-base-pedido?token=${token}`, {
                method: 'POST',
                body: formData
            });
            ...
            if (res.ok && data.success) {
                if (feedback) {
                    ...
                    feedback.innerHTML = `<strong>✅ Éxito:</strong> Se cargaron <strong>${data.count}</strong> artículos con columnas: <em>${data.headers.join(', ')}</em>`;
                }
                window.swalAlert?.('Base Actualizada', `Se importaron ${data.count} artículos en la base de pedidos de Trust correctamente.`, 'success');
                await window.loadTrustBaseStatus();
            } else {
                throw new Error(data.error || 'Error al procesar el archivo Excel.');
            }
        } catch (err) {
            console.error('[Trust] Error subiendo Excel:', err);
            if (feedback) {
                feedback.style.display = 'block';
                feedback.style.color = '#ef4444';
                feedback.innerHTML = `<strong>❌ Error:</strong> ${err.message}`;
            }
            window.swalAlert?.('Error al subir', err.message, 'error');
        } finally {
            if (uploadBtn) {
                uploadBtn.disabled = false;
                uploadBtn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Cargar Base de Pedidos';
            }
        }
    };
}
